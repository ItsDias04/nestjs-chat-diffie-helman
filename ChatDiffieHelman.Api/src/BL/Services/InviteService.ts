...
export class InviteService {
  ...
  async createInvite(
    ...
    if (!chat.users.find((u) => u.id === inviterId)) {
      throw new Error(
        `User with id ${inviterId} is not a member of chat with id ${chatId}`,
      );
    }
    const invite = await this.inviteRepository.findOne({
      where: {
        chat: { id: chatId },
        userSender: { id: inviterId },
        userReceiver: { id: userReceiverId },
      },
    });
    if (invite) {
    ...
    const userReceiver = await this.userRepository.findOne({
      where: { id: userReceiverId },
    });
    if (!userReceiver) {
  ...
  async respondToInvite(inviteId: string, accept: boolean): Promise<InviteDto> {
    const invite = await this.inviteRepository.findOne({
      where: { id: inviteId },
      relations: ['chat', 'userSender', 'userReceiver'],
    });
    if (!invite) {
