...
| Тег | Описание |
|-----|----------|
| **auth** | Аутентификация (login, Fiat-Shamir, BMC) |
...
1. **Используйте "Authorize"** один раз, чтобы не вставлять токен в каждый запрос
2. **Копируйте curl команды** из Swagger для использования в скриптах
3. **Смотрите Schema** для понимания структуры данных
4. **Сохраните swagger-spec.json** для документирования проекта
5. **Используйте фильтр** для быстрого поиска эндпоинтов
