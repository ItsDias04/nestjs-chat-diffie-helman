...
export class DiffieHellmanService {
   ...
   */
  connect(token: string, serverUrl: string = 'ws://localhost:3001'): void {
  ...
  async setActiveUsersAndStartKeyExchange(users: ActiveUser[]): Promise<void> {
    ...
    if (currentUserIndex === -1) {
      this.errorsSubject.next(
        'Текущий пользователь не найден в списке активных пользователей',
      );
      return;
    }
    ...
    try {
      ...
      this.errorsSubject.next(
        `Ошибка при подготовке к обмену ключами: ${error}`,
      );
  ...
  private async initiateKeyExchange(sortedUsers: ActiveUser[]): Promise<void> {
    try {
      ...
      if (!nextUser) {
        this.errorsSubject.next(
          'Не найден следующий пользователь для отправки ключа',
        );
        return;
      }

      this.keyExchangeStatusSubject.next(
        `Отправка ключа пользователю ${nextUser.id}...`,
      );
  ...
  private async handleReceivedMessage(message: DhMessage): Promise<void> {
    ...
    try {
      ...
      this.keyExchangeStatusSubject.next(
        `Получено сообщение на этапе ${message.stage}`,
      );
      ...
      const intermediateResult = await this.computeDiffieHellman(
        this.myPrivateKey,
        message.publicKey,
      );
      ...
      const newPublicKey =
        await this.derivePublicKeyFromSecret(intermediateResult);
      ...
      if (!nextUser) {
        this.errorsSubject.next(
          'Не найден следующий пользователь для продолжения обмена',
        );
        return;
      }

      this.keyExchangeStatusSubject.next(
        `Пересылка ключа пользователю ${nextUser.id}...`,
      );
   ...
   */
  private async computeDiffieHellman(
    ...
    try {
      ...
      return Array.from(sharedArray, (byte) =>
   ...
   */
  private async calculateFinalSharedSecret(
    ...
    try {
      this.sharedSecret = await this.computeDiffieHellman(
        this.myPrivateKey,
        finalPublicKey,
      );
