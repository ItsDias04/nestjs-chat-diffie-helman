...
import { ChatGateway } from 'src/WebApi/GateWays/ChatGateWay';
@Injectable()
export class MessageService {
  ...
  async getMessagesInChat(
    ...
    const messages = await this.messageRepository.find({
      where: { chatId: chatId },
      relations: ['chat', 'user'],
      order: { timestamp: 'ASC' }, // сортировка по дате (по возрастанию)
    });
  ...
  async addMessageToChat(
    ...
    if (!chat.users.find((u) => u.id === userId)) {
      throw new Error(
        `User with id ${userId} is not a member of chat with id ${chatId}`,
      );
    }
    const message = this.messageRepository.create(
      this.toMessageEntity(messageDto),
    );
    ...
    message.encryptionKeyIndex = chat.currentEncryptionKeyIndex;
    // message.dhStage = undefined;

    const resultDto = this.toMessageDto(
